...
uki.view.declare('uki.view.TextField', uki.view.Base, uki.view.Focusable, function(Base, Focusable) {
    ...
    */
    ...
    this._layoutDom = function(rect) {
        ...
        var parentThis = this;
        if (this._attributes) {
          uki.each(this._attributes, function(key, value) {
            if(key == 'readonly') {
              if(value == true) {
                parentThis._input.setAttribute(key);
              } else {
                parentThis._input.removeAttribute(key);
              }
            } else {
              parentThis._input.setAttribute(key, value);
            }
          });
        }
...
uki.Collection.addAttrs(['placeholder', 'attributes']);